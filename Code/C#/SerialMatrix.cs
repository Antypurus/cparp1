    ...
    public class SerialMatrix
    {
        ...
        public static SerialMatrix operator *(SerialMatrix A, SerialMatrix B)
        {
            ...
            //multiplication
            {
                ...
                {
                    int n = A.height;
                    int en = A.BLOCK_SIZE * (n / A.BLOCK_SIZE); /* Amount that fits evenly into blocks */

                    for (int kk = 0; kk < en; kk += A.BLOCK_SIZE)
                    {
                        for (int jj = 0; jj < en; jj += A.BLOCK_SIZE)
                        {
                            for (int i = 0; i < n; i++)
                            {
                                for (int k = kk; k < kk + A.BLOCK_SIZE; k++)
                                {
                                    for (int j = jj; j < jj + A.BLOCK_SIZE; j++)
                                    {
                                        retData[i * A.height + j] += A.getValue(i, k) * B.getValue(k, j);
                                    }
                                }
